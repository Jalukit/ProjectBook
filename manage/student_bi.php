...
if (@$_SESSION['status']!="1" && $_SESSION['status']!="3") {
  echo "";
         ...
         ?>
                <button type="button" class="btn btn-info float-right" onclick="window.location='add_student_bi.php'"><i class="fas fa-user-plus"></i> เพิ่มข้อมูล</button>
<?php } ?>    
          </div>
        </div>
      </div><!-- /.container-fluid -->
    ...
    @$sql="select*from studen ";
		...
		?>        
                  <tr>
                    ...
                    <td ><div align="center">  
                    <form action='check_student.php?id_p=<?php echo $result['id_p'] ?>&id_stu=<?php echo $result['id_stu'] ?>' method='post'>
