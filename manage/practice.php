...
<div class="wrapper">
 ...
 <nav class="main-header navbar navbar-expand navbar- navbar-light">
  ...
  <div class="content-wrapper">
    ...
    <section class="content">
      <div class="container-fluid">
        <div class="row">
          <div class="col-12">
            <div class="card">
              <div class="card-header">
                <h3 class="card-title">แสดงข้อมูลการปฏิบัติงานจิตอาสา</h3><br> 
                <h6> (หมายเหตุ มาหาพี่ วีระ กงลีมา ก่อนอันดับแรกเคาเตอรชั้น2 หรือเคาเตอร์บริการยืมคืนชั้นสอง หรือเจ้าหน้าที่ ศศธร มาศสถิตย์ และ วารุนี เพียรพจน์)</h6>
          ...
          <td ><div align="center">จำนวนผู้เข้าร่วม<div></td>
          <td ><div align="center">รวมเวลา<div></td>
         <!-- <td ><div align="center">แก้ไข<div></td>-->
          <td ><div align="center">การสมัคร<div></td>
    ...
    include "connect.php";
  	@$sql="select*from  project_participation ";
 // $sql="select*, COUNT('join_activities.id_p') as `items_total` FROM project_participation left join join_activities on project_participation.id_p=join_activities.id_p GROUP BY project_participation.id_p";
		$query=mysqli_query($conn,$sql);
		$numrow=mysqli_num_rows($query);
		$i=1;
		while($result=mysqli_fetch_array($query)){ 

      @$sql2="select*, COUNT('join_activities.id_p') as `items_total` FROM join_activities where join_activities.id_p='$result[id_p]'";
      $query2=mysqli_query($conn,$sql2);
		...
		?>        
                  <tr>
                    ...
                    <td align="center" class="align-middle"><a href="detail_data.php?id_p=<?php echo $result['id_p']?>"><button class="btn btn-info float-right">คลิก <i class='fas fa-eye'></i></button></a></td>
                    <td align="center" class="align-middle"><a href="detail_join.php?id_p=<?php echo $result['id_p']?>"><button class="btn btn-info"><?php echo $result2['items_total']?> คน <i class='fas fa-eye'></i></button></a></td>
                    <td align="center" class="align-middle">
<?php
 echo  duration($result['join_time'],$result['end_time']);
 ?>
                    </td>
                  <!--  <td align="center" class="align-middle"><a href='edit_practice.php?id_p=<?php echo $result['id_p']?>'><i class='fas fa-user-edit' style='font-size:48px;color:blue'></i></a></td>-->
                    <td align="center" class="align-middle">
                    <?php
    include "connect.php";
    @$sql3="select*, COUNT('join_activities.id_p') as `items_total` FROM join_activities left join project_participation on join_activities.id_p=project_participation.id_p where join_activities.id_p='$result[id_p]'";
    $query3=mysqli_query($conn,$sql3);
    $result3=mysqli_fetch_array($query3);
    
    if( $result3['items_total']==$result3['number_pract']){
        echo  "จำนวนผู้เข้าร่วมครบแล้ว";
    }else{
    @$s="select*from  join_activities left join project_participation on join_activities.id_p=project_participation.id_p where join_activities.id_p='$result[id_p]' and join_activities.id_stu='".$_SESSION['id_stu']."'";
...
echo "สมัครเรียบร้อยแล้ว";
		...
		?>  
                    <a href="add_join.php?id_p=<?php echo $result['id_p']?>" name="Submit"><button class="btn btn-info"><i class='fas fa-user-plus'></i> สมัครเข้าร่วมกิจกรรม </button></a>
<?php } }?>   
                    </td>
                  </tr>
    <?php } ?>     
