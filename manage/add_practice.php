...
include "connect.php"; 
$file=iconv("UTF-8","TIS-620",$_FILES["situation"]["name"]); //ขั้นตอนี่ 1 ตรวจสอบให้รับชืื่่อไฟล์ภาษาไทย
copy($_FILES["situation"]["tmp_name"],"picture/".$file);//ขั้นตอนี่ 2 กำหนดที่เก็บภาพในโฟล์เดอร์ ชื่่อ pictuer
$profile=$_FILES["situation"]["name"];
if($_POST['end_time'] < $_POST['join_time']){
  echo "<script> alert('คุณระบุเวลาไม่ถูกต้อง กรุณาตรวจสอบใหม่อีกครั้ง')</script>";
  echo "<script>history.back();</script>";
}else{
$sql="insert into project_participation (activity_name,situation,join_date,join_time,end_time,type,datail,number_pract) values
('".$_POST['activity_name']."','$profile','".$_POST['join_date']."','".$_POST['join_time']."','".$_POST['end_time']."','".$_POST['type']."','".$_POST['datail']."','".$_POST['number_pract']."')";
 ...
 <nav class="main-header navbar navbar-expand navbar-  navbar-light">
          ...
          <div class="col-12">
            <div class="card">
              <div class="card-header">
                <h3 class="card-title">เพิ่มข้อมูลการปฏิบัติงานจิตอาสา </h3>
  ...
  <form id="form1" name="form1" method="post" action="" onsubmit="return check_text()" enctype="multipart/form-data">
        ...
        <tr>
            ...
            <td colspan="3" align="left"><input name="activity_name" class="form-control" type="text" id="activity_name" size="50" required/>
           </td>
          </tr>
        <tr>
            <td align="left">สถานที่ :</td>
            <td colspan="3" align="left"><input name="situation" class="form-control" type="file" id="situation" size="50" required OnChange="Preview(this)"/>
           </td>
          </tr>
          <tr>
            <td width="150" align="left" scope="col">วันที่เข้าร่วม :</td>
            <td width="280" colspan="3" align="left" scope="col"><input name="join_date" class="form-control" type="date" id="join_date" size="50" required/></td>
          </tr>
          <tr>
            <td align="left">เวลาที่เข้าร่วม :</td>
            <td align="left">
            <input name="join_time" class="form-control" type="time" id="join_time" style="width:150px;" required/></td>
            
            <td align="center">ถึง</td>
            <td align="left"><input name="end_time" class="form-control" type="time" id="end_time" style="width:150px;" required/></td>
          </tr>
          <tr>
            <td align="left">ประเภทกิจกรรม :</td>
            <td colspan="3" align="left"><input name="type" class="form-control" type="text" id="type" size="50" required/>
            </td>
          </tr>
          <tr>
            <td align="left">รายละเอียดกิจกรรม :</td>
           <td colspan="3" align="left"><textarea name="datail" class="form-control" cols="50" rows="5"></textarea>
            </td>
          </tr>
          <tr>
            <td align="left">จำนวนผู้เข้าร่วม :</td>
           <td colspan="3" align="left"><input name="number_pract" style="width:100%;" class="form-control" type="text" id="number_pract" required onKeyUp="if(isNaN(this.value)){ alert('กรุณากรอกตัวเลข'); this.value='';}"/> 
            </td>
